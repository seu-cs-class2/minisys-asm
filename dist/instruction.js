...
exports.MinisysInstructions = (function () {
   ...
   */
    function patternWithParams(num) {
        if (num < 1) {
            return /^$/;
        }
        else {
    ...
    newInstruction('beq', '相等分支', 'if ((rt)=(rs)) then (PC)←(PC)+4+((Sign-Extend)offset<<2)', patternWithParams(3), [
        ...
        [25, 21, 'rs', function () { return register_1.regToBin(RegExp.$2); }, 'reg', ''],
        [20, 16, 'rt', function () { return register_1.regToBin(RegExp.$1); }, 'reg', ''],
        [15, 0, 'offset', function () { return utils_1.literalToBin(RegExp.$3, 18).slice(0, 16); }, 'offset', ''],
    ]);
    ...
    newInstruction('bgez', '大于等于0分支', 'if ((rs)≥0) then (PC)←(PC)+4+((Sign-Extend)offset<<2)', patternWithParams(2), [
        ...
        [15, 0, 'offset', function () { return utils_1.literalToBin(RegExp.$2, 18).slice(0, 16); }, 'offset', ''],
    ]);
    ...
    newInstruction('bgezal', '大于等于0分支（Link）', 'if ((rs)≥0) then ($31)←(PC)+4,(PC)←(PC)+4+((Sign-Extend) offset<<2)', patternWithParams(2), [
        [31, 26, 'op', function () { }, 'fixed', '000001'],
        [25, 21, 'rs', function () { return register_1.regToBin(RegExp.$1); }, 'reg', ''],
        [20, 16, 'rt', function () { }, 'fixed', '10001'],
        [15, 0, 'offset', function () { return utils_1.literalToBin(RegExp.$2, 18).slice(0, 16); }, 'offset', ''],
    ]);
    newInstruction('bltzal', '小于0分支（Link）', 'if ((rs)＜0) then ($31)←(PC)+4,(PC)←(PC)+4+((Sign-Extend) offset<<2)', patternWithParams(2), [
        [31, 26, 'op', function () { }, 'fixed', '000001'],
        [25, 21, 'rs', function () { return register_1.regToBin(RegExp.$1); }, 'reg', ''],
    ...
    newInstruction('j', '无条件跳转', '(PC)←((Zero-Extend)address<<2)', patternWithParams(1), [
        ...
        [25, 0, 'target', function () { return utils_1.literalToBin(RegExp.$2, 18).slice(0, 16); }, 'addr', ''],
    ]);
    ...
    // =================== NOP指令 ===================
    newInstruction('nop', '空转指令', 'do nothing', patternWithParams(0), [[31, 0, 'NOP', function () { }, 'fixed', '0'.repeat(32)]]);
