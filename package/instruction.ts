...
import { assert, binToHex, labelToBin, literalToBin, varToAddrBin } from './utils'
...
export const MinisysInstructions: Instruction[] = (function () {
  ...
  newInstruction('addi', '加立即数', '(rt)←(rs)+(sign-extend)immediate', paramPattern(3), [
    ...
    [25, 21, 'rs', () => regToBin(RegExp.$2), 'reg', ''],
    [20, 16, 'rt', () => regToBin(RegExp.$1), 'reg', ''],
    [15, 0, 'immediate', () => literalToBin(RegExp.$3, 16, true).slice(-16), 'immed', ''],
  ])
  ...
  newInstruction('andi', '按位与立即数', '(rt)←(rs)&(zero-extend)immediate', paramPattern(3), [
    ...
    [25, 21, 'rs', () => regToBin(RegExp.$2), 'reg', ''],
    [20, 16, 'rt', () => regToBin(RegExp.$1), 'reg', ''],
    [15, 0, 'immediate', () => literalToBin(RegExp.$3, 16).slice(-16), 'immed', ''],
  ])
  ...
  newInstruction('lui', '取立即数高16位', '(rt)←immediate<<16 & 0FFFF0000H', paramPattern(2), [
    ...
    [25, 21, 'rs', noop, 'fixed', '00000'],
    [20, 16, 'rt', () => regToBin(RegExp.$1), 'reg', ''],
    [15, 0, 'immediate', () => literalToBin(RegExp.$2, 16, true).slice(-16), 'immed', ''],
  ])
  ...
  newInstruction('beq', '相等分支', 'if ((rt)=(rs)) then (PC)←(PC)+4+((Sign-Extend)offset<<2)', paramPattern(3), [
    ...
    [25, 21, 'rs', () => regToBin(RegExp.$2), 'reg', ''],
    [20, 16, 'rt', () => regToBin(RegExp.$1), 'reg', ''],
    [15, 0, 'offset', () => labelToBin(RegExp.$3, 18, true, true).slice(-18, -2), 'offset', ''],
  ])
  ...
  newInstruction('j', '无条件跳转', '(PC)←((Zero-Extend)address<<2)', paramPattern(1), [
    ...
    [25, 0, 'target', () => labelToBin(RegExp.$1, 28, false).slice(-28, -2), 'addr', ''],
  ])
